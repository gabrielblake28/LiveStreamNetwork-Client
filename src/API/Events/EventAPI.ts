...
export class EventAPI implements IEventAPI {
    ...
    async GetFeaturedEvents(limit: number, page: number): Promise<IEvent[]> {
        const result = await this.query.get("/featured", { params: { limit, page}});
        return result.data;    }
    async GetTrendingEvents(limit: number, page: number): Promise<IEvent[]> {
        const result = await this.query.get("/trending", { params: { limit, page}});
        return result.data;    }
    async GetSponsoredEvents(limit: number, page: number): Promise<IEvent[]> {
        const result = await this.query.get("/sponsored", { params: { limit, page}});
        return result.data;    }
    async GetUpcomingEvents(limit: number, page: number): Promise<IEvent[]> {
        const result = await this.query.get("/upcoming", { params: { limit, page}});
        return result.data;    }
    async GetEventsWithMatchingUserIds(limit: number, page: number, userIds: string[]): Promise<IEvent[]> {
        throw new Error("Method not implemented.");
    }
    async GetEventsByTwitchCategory(category_id: string, limit: number, page: number): Promise<IEvent[]> {
        throw new Error("Method not implemented.");
    }
